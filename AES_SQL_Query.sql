...
CREATE TABLE Jobs (
	...
	Job_Title			varchar(20) UNIQUE	NOT NULL,
	CONSTRAINT [PKJobID] PRIMARY KEY (Job_ID ASC),
	CONSTRAINT [CHK_EMPJOB] CHECK (DATALENGTH(Job_Title) > 0 AND ISNUMERIC(Job_Title) = 0)
);
...
CREATE TABLE School (
	...
	School_Name		varchar(20)	UNIQUE NOT NULL,
...
CREATE TABLE Employer (
	...
	Employer_Name	varchar(20)	UNIQUE	NOT NULL,
	CONSTRAINT [PKEmployerID] PRIMARY KEY (Employer_ID ASC),
	CONSTRAINT [CHK_EMPEMPLOYERNAME] CHECK (DATALENGTH(Employer_Name) > 0 AND ISNUMERIC(Employer_Name) = 0)
);
